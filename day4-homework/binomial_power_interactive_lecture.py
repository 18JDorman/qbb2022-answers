...
# Not corrected
new_twodim_arr_noCor = numpy.zeros((len(probabilities), len(tosses)))
for i, prob in enumerate(probabilities):
    for j,toss in enumerate(tosses):
        power = run_experiment(prob, toss)
        new_twodim_arr_noCor[i,j] = power
...
# Corrected
new_twodim_arr_Cor = numpy.zeros((len(probabilities), len(tosses)))
for i, prob in enumerate(probabilities):
    for j,toss in enumerate(tosses):
        power = run_experiment(prob, toss, correct_the_pvalues = True)
        new_twodim_arr_Cor[i,j] = power

fig, ax = plt.subplots()
pallette = sns.color_palette("crest", as_cmap=True)
ax = sns.heatmap(new_twodim_arr_Cor, vmin = 0, vmax = 1, cmap = pallette, xticklabels = tosses, yticklabels = probabilities)
ax.set_xlabel("Number of Tosses")
ax.set_ylabel("Probability of Heads")
ax.set_title("Toss vs Probability on Power with P-Value Correction")
fig.savefig("PValCor.png")